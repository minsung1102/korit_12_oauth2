...
3. Avatar
  - 사용자 프로필 사진이나 이름 첫 문자를 동그랗게 띄워줄 때 사용합니다.
  - 이상의 코드에서는 그 사람 모양 아이콘 집어넣을려고 썼습니다.
...
export default function SignupPage() {
  ...
  return(
    ...
    <Container maxWidth="xs">
      <Box sx={{mt: 8, display: 'flex', flecDirection: 'column', alignItems: 'center'}}>
        <Paper elevation={3} sx={{ p: 4, width: '100%' }}>
          ...
          <Box component='form' onSubmit={handleSubmit}>
            ...
            >
              {signupMutation.isPending ? <CircularProgress size={24} color='inherit' /> : '회언가입'}
            </Button>
          </Box>
...
import { replace, useNavigate, useNavigate, useSearchParams } from "react-router-dom";
...
`http://localhost:5173/oauth2/callback?token=eyj....&email=kim1@gamil.com&name=김일&role=ROLE_USER`라는 전체 url을 return하게 되는데, 그 중에서 `?`뒷 부분에 있는 부분이 key-value형태와 유사하다는 것을 알 수 있습니다. token, email, name, role이라는 key를 갖고, `=` 뒷 부분이 value가 되며, 다음 key로 연결될 때 `&` 를 쓴 형태네요. qordpsemdptj `@PathVariable`을 통해서 endpoin 뒷부분의 id를 동적으로 가져올 수 있었습니다. `http://localhost:8080/api/vehicles/1`과 에서 `/1` 부분이었죠. 근데 search 라고 하는 엔드포인트를 만들어본 적도 있었습니다. repository 내에서 커스텀 메서드를 만들었을 때 였습니다. 그때 저희는 물음표 부분을 확인할 수 있었는데, 이처럼 `?` 뒷부분을 쿼리스트링(query string)이라고 칭합니다.
...
- 즉 `const [ searchParams ] = useSearchParams();라고 쓴 다음에 `searchParams.get('키이름');을 하면, 예를 들어 key가 token이라면 `http://localhost:5173/oauth2/callback?token=eyj....&email=kim1@gmail.com&name=김일&role=ROLE_USER`여기서 token= 뒷자리부터 & 앞까지의 값을 string으로 가져온다는 의미가 되겠습니다.

- `??` fks ? (null 병합 연산자)
- JS에서 null 오류를 잡기 위한건데 `??` 왼쪽 값이 null이면 오른쪽 값으로 대입한다는 겁니다.
`email : email ?? ''`를 썼다면 email key에 email value를 대입할건데 email value가 null/endefined의 경우에 공백('')을 입력하여 아예 email key가 존재하지 않아서 생겨나는 오류를 미연에 방지하겠다는 뜻입니다.

- 그러면 `||`랑 별반 차이 없어보일텐데
  - 차이점 : `||`은, `0, false, ''`를 전부 다 null값 처리해버리기 때문에 0 || '' 이라고 쓰면 ''가 입력될겁니다
  - `??`는 오직 null/undefined만 ?? 오른쪽 값으로 처리해줍니다.

- `{replace : true}` : OAuth2 로그인이 완료된 다음에 `/`를 통해 메인페이지로 진입하게 될텐데, 그때 뒤로가기 버튼 누르면 다시 callback page가 나오게 될겁니다. 그러면 불필요한 fetch가 일어날 수 있겠네요. 그거 막아주는 것입니다.
